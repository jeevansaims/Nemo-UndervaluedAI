        ...
        <div className="max-h-[88vh] overflow-y-auto">
          ...
          <section className="grid grid-cols-1 gap-3 px-6 pt-4 pb-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <MetricTile
              label="Margin Used"
              value={
                summary.marginUsed !== undefined
                  ? fmtCompactUsd(summary.marginUsed)
                  : "—"
              }
            ...
            <section className="px-6 pb-2">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[11px] text-neutral-500">
                  {sortedBreakdown.length} day
                  {sortedBreakdown.length === 1 ? "" : "s"}
                </span>
              </div>
          ...
          {/* STRATEGY CARDS - New card-based view */}
          {(() => {
            ...
            const strategyList = Array.from(strategyGroups.entries()).map(
              ([strategy, data]) => {
                const winRate =
                  data.trades.length > 0
                    ? (data.winCount / data.trades.length) * 100
                    : 0;
                const romPct =
                  data.totalMargin > 0
            ...
            const winners = strategyList.filter((g) => g.totalPL > 0);
            const losers = strategyList.filter((g) => g.totalPL <= 0);
            ...
            return (
              <section className="px-6 pb-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[11px] text-neutral-500">
                    {strategyList.length} strateg
                    {strategyList.length === 1 ? "y" : "ies"}
                  </span>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  {winners.length > 0 && (
                    <div className="space-y-2">
                      <h5 className="text-xs font-medium text-emerald-400/80 uppercase tracking-wide">
                      ...
                      <div className="grid gap-2 sm:grid-cols-2">
                        {winners.map((group) => (
                          <div
                            key={group.strategy}
                            className="rounded-xl border border-neutral-800 bg-neutral-900/60 px-4 py-3 hover:bg-neutral-800/60 transition-colors"
                          >
                  ...
                  {losers.length > 0 && (
                    <div className="space-y-2">
                      <h5 className="text-xs font-medium text-red-400/80 uppercase tracking-wide">
                        {winners.length > 0 ? "Losers" : "All Trades"} (
                        {losers.length})
                      </h5>
                      <div className="grid gap-2 sm:grid-cols-2">
                        {losers.map((group) => (
