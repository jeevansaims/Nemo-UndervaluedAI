...
export async function GET(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    if (dateSets.length === 0) {
         return NextResponse.json({ slug, range, points: [] as PerfPoint[] });
    }
    ...
    const bench0 = benchMap.get(startDate) ?? 1; // avoid /0 if benchmark missing (though dates intersection should prevent this)
    ...
    return NextResponse.json({ slug, range, benchmark: benchSymbol, points });
  } catch (e: any) {
